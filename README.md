...
1. `PIIMasker.mask_text()` regex-detects email/phone/self-introduced-name/street-address shapes and replaces each with `[EMAIL]`/`[PHONE]`/`[NAME]`/`[ADDRESS]` before anything reaches the LLM. Not a full NER model (see `services/pii_masker.py`'s own docstring for the precision/recall trade-offs of each pattern), but real detection against arbitrary input, not a hardcoded sample-string list.
...
5.6. A passive faithfulness check logs a warning if the reply omits a key fact an agent result carried (an order ID, a recommended product's name) - never blocks or changes the reply, just a signal worth a human glancing at the trace for.
...
| LLM provider (hybrid/cloud) | `ROUTER_PROVIDER`, `AGENT_REASON_PROVIDER`, `AGENT_INTERPRET_PROVIDER`, `GENERATIVE_PROVIDER`, `EMBEDDING_PROVIDER`, `GEMINI_API_KEY`, `GEMINI_API_BASE_URL`, `GEMINI_*_MODEL` | Each role independently defaults to `local` (Ollama); set to `gemini` to route that role to Gemini instead - see `services/llm_inference.py::_resolve_role()`. `EMBEDDING_PROVIDER` is the odd one out: unlike the other four (stateless per-request), embeddings feed a *persistent* FAISS index - switching it isn't safe without also clearing `docs/rag_data/`, or letting `RAGService`'s own provenance guard (`services/rag.py`) discard a mismatched store for you |
...
| Destructive-action confirmation | Ownership was enforced, but a single LLM tool-call decision alone was still sufficient to cancel/delete a real order - a prompt-injection success or a reasoning mistake, not just a routing mistake, could execute it. `OrderTrackingAgent` now asks first (`needs_confirmation` on its result) instead of calling `cancel_order()`/`delete_order()` immediately; the customer's next reply is matched against a fixed, deterministic yes/no keyword pattern - not a second LLM call, so it can't itself be talked into confirming something it was never asked - and fails closed (ambiguous = not executed) same as an explicit decline. | `services/agents/order_tracking_agent.py`, `services/orchestrator.py` (`pending_confirmation`) | **Enabled** |
| Faithfulness check on generated replies | `call_generative()`'s synthesis step could drop or misstate a structured detail (an order ID, a recommended product's name) with nothing surfacing it. Passive/logging-only by design - flags a plausible-looking-but-wrong reply for a human to check in the trace, rather than risk a false positive blocking a reply that's actually fine. | `services/orchestrator.py::_log_faithfulness_check()`, step 5.6 | **Enabled** |
...
3. `services/llm_inference.py` - the four LLM entry points (`call_agent_reason`, `call_agent_interpret`, `call_generative`, `call_embeddings`), each with a docstring explaining its contract and fallback behaviour. Routing itself is `call_task_decomposer()` (step 3 above).
...
- ~~`GET /api/v1/chat/{session_id}/history` and `DELETE /api/v1/chat/{session_id}` are mentioned in `api/routers/chat.py`'s docstring but not implemented~~ - fixed: both are implemented, reading/clearing the same in-memory `conversation_history_db`/`agent_state_store` the orchestrator already maintained; each requires a `user_id` query param matching the session's original owner (`AgentOrchestratorService.session_owner`), 404ing identically for "wrong owner" and "no such session" (see `api/README.md`).
...
- ~~`ProductRecommendationAgent` hardcoded the recommended `product_id`/`name`/`price` (see its docstring) - only the RAG-matched description snippet and the customer-history framing were real~~ - fixed: the RAG-fallback path now reads the real `product_id` `ingest_product_catalog()` (`services/data_pipeline.py`) already stamps into the matched chunk's metadata, and looks it up live via `EcommerceClient.get_item()` - same real-catalog-data guarantee the direct `searchItems` tool path already had, and fails closed (no recommendation) rather than fabricating one if the looked-up item no longer exists.
...
- ~~Embeddings weren't part of the hybrid local/Gemini provider design - `call_embeddings()` always used Ollama regardless of the chat-role `*_PROVIDER` settings, and silently returned a zero-vector on any failure instead of raising~~ - fixed: `EMBEDDING_PROVIDER` (default `local`) now mirrors the other four roles (see the Configuration table above), and `call_embeddings()` raises `EmbeddingUnavailableError` on failure instead of a zero-vector fallback - a silent all-zero embedding doesn't error against FAISS's `IndexFlatL2`, it just makes every document "equidistant," turning a real backend outage into near-arbitrary RAG results with no visible failure. Every caller (`services/data_pipeline.py`'s ingestion methods, `GeneralPurposeAgent`/`ProductRecommendationAgent`'s RAG lookups, `api/dependencies.py`'s `warm_up_services()`) now handles that exception explicitly, degrading gracefully (skip the affected chunk/product, or answer "no RAG match") instead of crashing the whole ingestion run or the API's startup. Because embeddings feed a persistent index (unlike the other four, stateless-per-request roles), `RAGService` also stamps a provenance marker recording which provider/model built the store, and discards - rather than silently mixes into - a persisted store built with a different one.
